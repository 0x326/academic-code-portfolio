...
for ITERATION_COUNT in {1..5}; do
    ...
    perf stat -e "${PERF_STAT_EVENTS}" ./unroll standard 2>> "${LAB_NUM}-standard.perfstat"
done

for ITERATION_COUNT in {1..5}; do
    echo "Run ${ITERATION_COUNT}: ./unroll unrolled"
    perf stat -e "${PERF_STAT_EVENTS}" ./unroll unrolled 2>> "${LAB_NUM}-unrolled.perfstat"
done

for ITERATION_COUNT in {1..5}; do
    echo "Run ${ITERATION_COUNT}: ./unroll-with-funroll standard"
    perf stat -e "${PERF_STAT_EVENTS}" ./unroll-with-funroll standard 2>> "${LAB_NUM}-standard-with-funroll.perfstat"
done
